...
(defn write-to-redis [campaigns ads redis-host redis-port]
  ...
  (with-open [ad-to-campaign-o (clojure.java.io/writer "ad-to-campaign-ids.txt")]
    (binding [*out* ad-to-campaign-o]
      (let [campaigns-ads (map vector campaigns (partition 10 ads))]
        (redis/with-server {:host redis-host :port redis-port}
                           (redis/flushall)
...
(defn get-stats [redis-host redis-port]
...
(defn gen-ads [redis-host redis-port]
  (redis/with-server {:host redis-host :port redis-port}
                     (let [campaigns (redis/smembers "campaigns")
...
(defn run [throughput with-skew? kafka-hosts redis-host redis-port]
  (println "Running, emitting" throughput "tuples per second.")
  (let [ads (gen-ads redis-host redis-port)
...
(defn do-new-setup [redis-host redis-port]
  ;; Hook up the redis DB
...
(defn check-correct [redis-host redis-port]
...
(defn do-setup [kafka-hosts redis-host redis-port]
  (let [{campaigns :campaigns ads :ads} (load-ids)]
    (if (or (nil? campaigns) (nil? ads))
      ;; Create new ids
      (let [campaigns (make-ids num-campaigns)
            ads (into [] (make-ids (* num-campaigns 10)))]
        (write-to-redis campaigns ads redis-host redis-port)
        (write-to-kafka ads kafka-hosts)
...
(defn get-conf [confPath]
  (let [conf (yaml/parse-string (slurp confPath))
    ...
    (println {:redis-host redis-host :redis-port redis-port :kakfa-brokers kafka-hosts})
    {:redis-host redis-host :redis-port redis-port :kakfa-brokers kafka-hosts}))
...
(def cli-options
  [["-s" "--setup" "Set up for catchup-simulation-mode (or re-setup if the .txt files exist)"]
   ...
   ["-a" "--configPath PATH" "Path to config yaml file"
    :default "./resources/benchmarkConf.yaml"
...
(defn -main
  ...
  (let [{:keys [options arguments errors summary]} (cli/parse-opts args cli-options)
    ...
    (cond
      ...
      (:setup options)                        (do-setup kafka-hosts redis-host redis-port)
      (:check options)                        (check-correct redis-host redis-port)
      (:new options)                          (do-new-setup redis-host redis-port)
      (:run options)                          (run (:throughput options) (:with-skew options) kafka-hosts redis-host redis-port)
      (:get-stats options)                    (get-stats redis-host redis-port)
